...
try {
    ...
    if (isset($_POST["buscar_persona"])) {
        ...
        if (!empty($errores)) {
            ...
            $consulta1->execute([$coleccionista,$coleccionista,$casoFormal,$casoTelematico, $identificador,$identificador,$identificador]);
        }
    }elseif(isset($_POST["buscarCasosActivos"])){
        $consulta1 = $conexiondb->prepare("SELECT usuarios.*,trabajadores.nombre AS trabajador_nombre  
                                                     FROM usuarios 
                                                     JOIN trabajadores ON trabajadores.id = ?
                                                     WHERE persona_id= ? 
        ...
        $consulta1->execute([$coleccionista,$coleccionista,$casoFormal,$casoTelematico]);
    } else {
        // Consulta sin filtros
        $consulta1 = $conexiondb->prepare('SELECT usuarios.*,trabajadores.nombre AS trabajador_nombre 
                                                 FROM usuarios 
                                                 JOIN trabajadores ON trabajadores.id = :coleccionista
                                                 WHERE persona_id= :coleccionista 
                                                 AND intervencion IN (:casoFormal, :casoTelematico) ');
        $consulta1->execute([':casoFormal' => $casoFormal, ':casoTelematico' => $casoTelematico, ':coleccionista' => $coleccionista]);
    }
